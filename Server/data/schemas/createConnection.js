...
const createTestReportModel = require('./testReportSchema');
const { logger } = require('../../app-logger');

let Test, Topic, Question, User, TestReport;
const createDataConnection = async () => {
    ...
    createConnection(`mongodb://${domain}:${db_port}/${database_records_name}`).asPromise().then((con) => {
        TestReport = createTestReportModel(con);
    });
}

const getModels = () => ({ Test, Topic, Question, User, TestReport });
module.exports = { createDataConnection, Test, Topic, Question, User, TestReport, getModels };